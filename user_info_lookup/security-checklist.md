...
- [x] ユーザー選択(`USER_SELECT`)フィールドをクリアする際は値の型(配列)に合わせて空配列(`[]`)にしており、文字列フィールドと型を混同していない
- [x] `kintone.app.record.set()`でフィールド値を空文字列にクリアすると`value`キー自体が消える(`undefined`になる)というkintoneの実挙動(`biz_code_search`の開発時に検証環境で確認済み、idea.md参照)を踏まえ、クリア後の値を扱う既存コード(`extractUserCode`)は`typeof`判定のみで影響を受けないことを確認済み
