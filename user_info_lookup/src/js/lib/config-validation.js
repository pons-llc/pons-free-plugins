(function (root) {
  ...
  // 元フィールドは文字列1行またはユーザー選択のみ(idea.md「文字列１行またはユーザー選択フィールドから
  // 転記する」)。
  const VALID_SOURCE_FIELD_TYPES = ['SINGLE_LINE_TEXT', 'USER_SELECT'];
  // 出力先は文字列フィールドのみ(idea.md「挿入先は文字列フィールド」)。
  ...
  const validateRows = (rows, fieldInfoByCode) => {
    ...
    rows.forEach((row, index) => {
      ...
      if (!row || !isNonEmptyString(row.sourceFieldCode)) {
        ...
        errors.push(
          `${label}: 元フィールドは文字列1行またはユーザー選択のみ選択できます。`,
        );
      }
      ...
      mappings.forEach((mapping, mappingIndex) => {
        ...
        if (
          !mapping ||
          !UserAttributes.ATTRIBUTE_KEYS.includes(mapping.attribute)
        ) {
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    root.UserInfoLookup.ConfigValidation = ConfigValidation;
  }
