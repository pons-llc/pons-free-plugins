...
//
// このテストの主眼は「元フィールド」プルダウンが実際に文字列1行/ユーザー選択のみに絞り込まれること、
...
const PLUGIN_NAME = 'user_info_lookup';
...
describe('設定画面(実環境)', () => {
  ...
  test('設定画面が開き、設定行を追加すると各プルダウンの選択肢が実際に絞り込まれる', async () => {
    ...
    expect(heading).toContain('ユーザー情報ルックアッププラグイン');

    await page.click('#js-row-add');
    const rows = await page.$$('.js-row');
    const newRow = rows[rows.length - 1];

    // 元フィールド: 文字列1行/ユーザー選択のみが選択肢に出ているはず(config.js冒頭の
    // SOURCE_FIELD_TYPESの絞り込みが実際に効いているかの回帰確認)。fixtures.jsで用意した
    // 「uil_user_code」(文字列1行)と、TEST_APP_ID_1に既存の「ユーザー選択」が含まれ、数値型の
    // 「数値」は含まれない。
    const sourceOptionValues = await newRow.$$eval(
      '.js-source-field option',
      (options) => options.map((o) => o.value).filter((v) => v !== ''),
    );
    expect(sourceOptionValues).toContain('uil_user_code');
    expect(sourceOptionValues).toContain('ユーザー選択');
    ...
    expect(spaceOptionValues).toContain('uil_button_space');

    // 転記項目を1件追加し、出力先フィールドの選択肢が文字列1行のみに絞り込まれていることを確認する
    // (config.js冒頭のDESTINATION_FIELD_TYPESの絞り込みが実際に効いているかの回帰確認)。
    await (await newRow.$('.js-mapping-add')).click();
    const destinationOptionValues = await newRow.$$eval(
      '.js-mapping-destination option',
      (options) => options.map((o) => o.value).filter((v) => v !== ''),
    );
    expect(destinationOptionValues).toContain('uil_out_name');
    ...
    await (await newRow.$('.js-button-space')).select('uil_button_space');
    const attributeSelect = await newRow.$('.js-mapping-attribute');
    await attributeSelect.select('name');
    const destinationSelect = await newRow.$('.js-mapping-destination');
    await destinationSelect.select('uil_out_name');
