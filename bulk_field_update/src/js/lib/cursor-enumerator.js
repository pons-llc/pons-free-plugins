(function (root) {
  ...
  // 一覧画面の絞り込み条件に一致する対象レコードを、レコードカーソルAPIで列挙するための
  // orchestrationロジック。実際のAPI呼び出しは依存性注入(deps)にして、
  // kintoneに依存しない形でテストできるようにしている。
  // age_grade_field_update/related_record_summaryと同一実装、名前空間のみ変更。
  //
  // deps:
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    root.BulkFieldUpdate.CursorEnumerator = CursorEnumerator;
  }
