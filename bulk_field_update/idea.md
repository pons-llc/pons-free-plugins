...
- **ルックアップの「ほかのフィールドのコピー」設定でコピー先に指定されているフィールド**(確定・
  2026-08-09バグ修正): ルックアップフィールド自体(`field.lookup`が設定されているフィールド)が
  自動転記する**コピー先フィールド**(`lookup.fieldMappings[].field`で指定される、ルックアップ
  フィールドとは別のフィールド)は`field.lookup`を持たず、通常のSINGLE_LINE_TEXT/NUMBER等として
  見えるため、型による判定だけでは除外できない(実際のバグ報告により発覚)。コピー先フィールドを
  一括更新の対象にすると、次回のルックアップ実行時に上書きされてしまいコピー元との整合性が
  壊れるため、`js/lib/field-eligibility.js`の`collectLookupCopyDestinationCodes()`で全フィールドの
  lookup設定からコピー先フィールドコードを収集し、`listEligibleFields()`で除外している。
  コピー先フィールド自体は本プラグインの対象にはできない(REST APIドキュメント「1件のレコードを
  更新する」の制限事項で、ルックアップ元からコピーされるフィールドは更新不可と明記されている)。

### ルックアップフィールド自体の扱い(確定・2026-08-09)

ルックアップフィールド自体(`field.lookup`が設定されているフィールド)は、上記のコピー先除外とは
別扱いで**対象フィールドに含める**。ただし通常のフィールドとは書き込み方が異なる。

- 当初はルックアップフィールドも一律対象外にしていたが、ユーザーからの指摘「TYPEにルックアップが
  ない」を受けてkintoneドキュメントMCPで調査した結果、ルックアップフィールドの値自体はPUT APIで
  更新可能であり、しかもその書き込みがコピー先フィールドの自動転記を再実行させる(kintone公式Tips
  「ルックアップの更新を自動で行う」で確認済み)ことが分かった。この挙動を活かし、「ルックアップ
  フィールドを対象にすると、そのレコードの現在値のまま書き戻すことで関連レコードの最新情報へ
  コピー先フィールドを一括で再取得できる」機能として実装した。
- 設定画面(`js/config.js`)の種類欄には、通常の型ラベルではなく「ルックアップ・◯◯」
  (例: 「ルックアップ・文字列(1行)」)と表示し、通常フィールドと見分けられるようにする。
- 確認ダイアログ(`js/bulk-update.js`)では、ルックアップフィールドの行に**値の入力欄を出さない**
  (`FieldEligibility.inputKindOf()`が`'LOOKUP_REFRESH'`を返すフィールドは`buildValueControl`を
  呼ばず、代わりに「現在の値のまま更新します(値の入力はありません。関連レコードの情報を
  再取得します)。」という注記(`.bfu-lookup-note`)を表示する)。「このフィールドを更新する」
  チェックボックスはルックアップフィールドにも表示され、OFFにすればそのレコードの再取得をスキップできる。
- 書き戻し時は、レコードごとに**そのレコード自身が現在持っている値**をルックアップフィールドへ
  そのまま書き戻す(共有の固定パッチ値ではなく、レコードカーソルで取得した`record[コード].value`を
  使う)。そのため、カーソル作成時の`fields`パラメーターには通常フィールド用の`$id`/`$revision`/
  レコード番号に加えて、ルックアップ再取得の対象候補フィールドコードも含める
  (`js/bulk-update.js`の`buildCursorFields`)。
- 最終確認ダイアログでは、ルックアップフィールドの行に値そのものではなく
  「現在の値のまま更新(関連レコードを再取得)」という説明文を表示する
  (`js/lib/value-summary.js`は値の入力を伴わないため、この文言は`js/bulk-update.js`側で組み立てる)。
...
| LOOKUP_REFRESH | ルックアップ | (入力欄なし、注記のみ) | (該当なし、常にレコード自身の現在値を書き戻す) |

`inputKindOf()`はフィールドオブジェクト全体を受け取った場合、型に関わらず`field.lookup`が
設定されていれば`'LOOKUP_REFRESH'`を優先して返す(例えばルックアップの参照先キーが文字列型でも、
ラジオボタン型の選択肢ではなくLOOKUP_REFRESH扱いになる)。
...
- 対象フィールドごとに1行を表示し、各行は次の2つで構成する(`js/bulk-update.js`の
  ...
  2. 書き込む値の入力欄。空欄から都度入力し、既定値は持たない。DATE型は`<input type="date">`、
     ...
     並べる。ただしルックアップフィールド(上記「ルックアップフィールド自体の扱い」参照)は
     入力欄を出さず、「現在の値のまま更新します」という注記のみを表示する。
...
- `field-eligibility.js`: 対象外フィールドの判定(ルックアップのコピー先フィールドの除外を含む)・
  型ごとの入力欄分類(ルックアップフィールド自体は`'LOOKUP_REFRESH'`として分類)。
