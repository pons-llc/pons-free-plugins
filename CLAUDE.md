...
- `site/` — Cloudflare Pagesの公開ディレクトリ(サイト名「GovAppsプラグイン」。構成・デザインは`site-wire-frame.md`と姉妹サイト`~/Documents/govapps`を踏襲)。
  ...
  - `solutions/<slug>/index.html` — 検索意図(「kintoneで○○したい」)別の解決策ページ。「こんな方におすすめ」+「対応するプラグイン」(複数可)+「あわせて見たい解決策ページ/記事」の構成で揃える。`solutions/index.html`が一覧ハブ(SEO設計の詳細は`seo_idea.md`参照)。手順の実行版は`.claude/skills/solutions-page/SKILL.md`を参照(解決策ページを執筆する/S0xページを作ると依頼されたときに使う)
  - `articles/<slug>/index.html` — 特定のプラグイン/ユースケースを深掘りする記事ページ。`seo_idea.md`の「原因 → 標準機能でできるか → JavaScriptならどうするか → プラグインならどうするか → GovApps」という流れに沿って、「こんな方におすすめ」「標準機能でどこまでできるか」「JavaScript自作・他社(有料)プラグインとの比較、なぜGovAppsプラグインか(無料・広告なし・OSS・セキュリティレビュー済みという差別化を明示する。他社プラグインを名指しで批判はしない)」「プラグインでの実現方法(設定画面スクショ)」「実行結果(作成結果スクショ)」「対応するプラグイン」「あわせて見たい記事・解決策ページ」の構成で揃える。**「比較・なぜGovAppsか」のセクションは省略しないこと**(GovAppsの強みを伝える記事という目的上、プラグインの使い方だけを書いた記事にしない)。`articles/index.html`が一覧ハブ、`articles/organization-inquiry/`が実装例
...
1. **kintoneドキュメントMCP(`kintone_doc`)を必ず参照する** — JavaScript API/REST APIの仕様や挙動、注意点は実装前に必ずMCP経由で確認する。記憶だけで実装しない。
   ...
   - **既知の落とし穴: `kintone.createDialog()`が返す`dialog.show()`のPromiseは、ダイアログが表示された時点ではなく、ユーザーが閉じた時点で解決される。** そのため、集計・保存等の処理を`kintone.showLoading('VISIBLE')`→(REST呼び出し)→`kintone.createDialog()`→`dialog.show()`→`kintone.showLoading('HIDDEN')`と丸ごと1つの`try/finally`で囲むと、ローディングのオーバーレイが「ダイアログを閉じるまで」表示されっぱなしになり、オーバーレイに遮られてユーザーがダイアログを閉じられず、ローディングが永久に消えなくなる(実例: `budget_meter`の集計ボタンで発生。`showLoading('HIDDEN')`は、REST呼び出し等の「データが揃うまで」の区間だけを囲み、ダイアログの構築・`dialog.show()`の前に呼ぶこと)。
