...
- [x] クリアボタン(法人番号・法人名・転記項目をまとめて空にする)は押下時に`confirm()`でユーザーに再確認させ、キャンセルした場合は何もクリアしない
- [x] `kintone.app.record.set()`でフィールド値を空文字列にクリアすると`value`キー自体が消える(`undefined`になる)というkintoneの実挙動を実際に検証環境で確認し、クリア後の値を読む箇所(`field.value.trim()`等)は`field && field.value`で存在確認してからアクセスするよう修正済み(idea.md参照。修正しない場合、法人名フィールドがクリア済みの状態で「法人名から検索」ボタンを押すと`TypeError`が発生しうる)
