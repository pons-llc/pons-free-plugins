...
最終確認日: 2026-08-21 / 対象: 一覧インライン編集(`app.record.index.edit.submit`)対応の追加
...
- 一覧画面のインライン編集(`app.record.index.edit`)で基準フィールドを直接変更して保存した場合は本プラグインも再計算する(2026-08-21対応)。ただしCALCフィールドをオフセット参照に使うルールは、上記「一覧インライン編集固有のリスク」の理由により計算をスキップし出力先フィールドが古い値のまま残り得る(意図的な安全策として明記済み)
