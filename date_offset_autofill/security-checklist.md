...
- [x] 2026-08-21追加: 設定時のcautionに加え、実際にスキップが発生した保存時にも`app.record.index.edit.submit`ハンドラーが`alert()`で通知するようにした。`event.error`(保存をキャンセルする画面上部エラー)ではなく`alert()`を使うことで、一部のルールが計算できなかった旨を伝えつつ保存自体は妨げない設計にしている
