(async (PLUGIN_ID) => {
  ...
  const NS = window.OrgLookup;

  // 元メモ「ユーザーと同様の仕様」(user_info_lookupの「文字列１行またはユーザー選択フィールドから
  // 転記する」)に対応する組織版。
  const SOURCE_FIELD_TYPES = ['SINGLE_LINE_TEXT', 'ORGANIZATION_SELECT'];
    ...
    config.rows.forEach((row, rowIndex) => {
      ...
      const renderMappingList = () => {
        ...
        row.mappings.forEach((mapping, mappingIndex) => {
          ...
          NS.OrgAttributes.ATTRIBUTES.forEach((attribute) => {
            const optionEl = document.createElement('option');
