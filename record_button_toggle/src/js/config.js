(async (PLUGIN_ID) => {
  ...
  const NS = window.RecordButtonToggle;
    ...
    config.rules.forEach((rule, ruleIndex) => {
      ...
      const renderClauseValueControl = (containerEl, clause) => {
        ...
        if (DATE_INPUT_TYPE[clause.fieldType]) {
          ...
          controlEl.className =
            'kintoneplugin-input-text rbt-clause-value js-clause-value';
          ...
          controlEl.className =
            'kintoneplugin-select rbt-clause-value js-clause-value';
          const values =
      ...
      targetEl.value = rule.targetButton;
      ...
      targetEl.addEventListener('change', () => {
        rule.targetButton = targetEl.value;
      });
