...
// 同じ方針)。TEST_APP_ID_2にDROP_DOWN型のフィールドが無ければfixtures.jsで冪等に追加する。
...
const PLUGIN_NAME = 'record_button_toggle';
...
describe('設定画面(実環境, スクリーンショット取得用)', () => {
  ...
  test('設定画面が開き、対象ボタン・条件(ドロップダウン)を選択できる', async () => {
    const pageErrors = [];
    ...
    expect(heading).toContain('追加・編集・コピーボタン');
    ...
    expect(targetOptionValues).toEqual(['ADD', 'EDIT', 'COPY']);

    await (await ruleRow.$('.js-rule-target')).select('EDIT');
    ...
    await (await ruleRow.$('.js-rule-action')).select('HIDE');
