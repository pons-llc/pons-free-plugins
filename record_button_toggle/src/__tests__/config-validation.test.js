...
describe('ConfigValidation.validateRules', () => {
  ...
  test('正しいALWAYSルールを許可する(条件不要)', () => {
    const result = ConfigValidation.validateRules([
      { targetButton: 'EDIT', mode: 'ALWAYS', action: 'SHOW' },
    ]);
    expect(result).toEqual({ valid: true, errors: [] });
  });

  test('対象ボタンが不正な値なら拒否する', () => {
    const result = ConfigValidation.validateRules([
      matchRule({ targetButton: 'DELETE' }),
    ]);
    expect(result.valid).toBe(false);
  });

  test('対象ボタン(COPY)は許可する', () => {
    const result = ConfigValidation.validateRules([
      matchRule({ targetButton: 'COPY' }),
    ]);
    expect(result).toEqual({ valid: true, errors: [] });
  });
