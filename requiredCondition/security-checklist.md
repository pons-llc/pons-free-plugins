# requiredCondition(条件つき必須プラグイン) セキュリティチェックリスト
...
- [x] kintone内部のid/class属性やDOM構造に依存せず、JavaScript API(`kintone.plugin.app.getConfig/setConfig()`, `kintone.events.on()`)を使用している
...
- [x] `kintone.plugin.app.setConfig()`に保存しているのは条件フィールド・条件値・必須対象フィールドのマッピングのみで、認証情報や機密情報は含まれない
...
- 条件フィールドと同一フィールドを必須対象に設定した場合など、想定外の組み合わせでの挙動
