...
// 作成(relatedApp等)は不要。「検索先のキーフィールド」にはlike演算子が使える文字列(1行)/リンク
// フィールドを1つ新設し、シードレコードの用意・検索の一意性確認をしやすくするためunique設定も
// 付けている(部分一致検索自体はunique設定を要求しない)。
//
// フィールド設計:
//   - selfKeyFieldCode(自レコードの検索キー) には既存の「文字列__1行_」を使う(新設不要)。
//   - otherKeyFieldCode(検索先のキーフィールド)として `slk_key` (文字列1行)を新設する。
