...
- [x] クリアボタン(検索キー・転記項目をまとめて空にする)は押下時に`confirm()`でユーザーに再確認させ、キャンセルした場合は何もクリアしない(`js/desktop.js`/`js/mobile.js`)
- [x] `kintone.app.record.set()`でフィールド値を空文字列にクリアすると`value`キー自体が消える(`undefined`になる)というkintoneの実挙動(`biz_code_search`の開発時に検証環境で確認済み、idea.md参照)を踏まえ、クリア後の値を扱う既存コードは偽値判定のみで`.value`にメソッドを直接呼ぶ箇所が無いことを確認済み
