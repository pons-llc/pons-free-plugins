(async (PLUGIN_ID) => {
  ...
  const NS = window.GroupFieldToggle;
  ...
  const groupFields = fieldsByType.GROUP || [];
  if (groupFields.length === 0) {
    document.getElementById('js-no-group-fields').hidden = false;
    ruleAddButtonEl.disabled = true;
  }
    ...
    config.rules.forEach((rule, ruleIndex) => {
      ...
      const renderClauseValueControl = (containerEl, clause) => {
        ...
        if (DATE_INPUT_TYPE[clause.fieldType]) {
          ...
          controlEl.className =
            'kintoneplugin-input-text gft-clause-value js-clause-value';
          ...
          controlEl.className =
            'kintoneplugin-select gft-clause-value js-clause-value';
          const values =
      ...
      buildFieldCodeOptions(targetEl, 'GROUP', rule.targetFieldCode);
  ...
  ruleAddButtonEl.addEventListener('click', () => {
    config.rules.push({
      targetFieldCode: groupFields.length > 0 ? groupFields[0].code : '',
      mode: 'MATCH',
