(function (root) {
  ...
  const validateRules = (rules) => {
    ...
    rules.forEach((rule, index) => {
      ...
      if (!rule || !isNonEmptyString(rule.targetFieldCode)) {
        errors.push(`${label}: 対象グループフィールドが選択されていません。`);
      }

      if (!rule || !ACTIONS.includes(rule.action)) {
        errors.push(`${label}: 動作(開く/閉じる)の指定が不正です。`);
      }
  ...
  if (typeof module !== 'undefined' && module.exports) {
    ...
    root.GroupFieldToggle.ConfigValidation = ConfigValidation;
  }
