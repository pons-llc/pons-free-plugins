...
// バリデーション・保存内容の読み直しを確認する(静的HTML・単体テストでは検知できない実環境固有の
// 挙動、CLAUDE.mdの開発方針1参照)。公開サイト用のスクリーンショットもここで撮る。
//
// 事前準備:
...
describe('設定画面(実環境)', () => {
  ...
  test('ルールを追加し、フィルター選択肢・型別ソース欄の出し分け・バリデーション・保存内容の読み直しができる', async () => {
    ...
    // フィルター条件のアクション名選択肢に、プレビューのプロセス管理設定(REST API)から
    // 取得したアクション名が含まれる。
    const actionOptionValues = await ruleRow.$$eval(
      '.js-filter-action option',
      (options) => options.map((o) => o.value).filter((v) => v !== ''),
    );
    expect(actionOptionValues).toContain(fixtures.PROCESS_ACTIONS.start.name);
    expect(actionOptionValues).toContain(
      fixtures.PROCESS_ACTIONS.complete.name,
    );
    await (
      await ruleRow.$('.js-filter-action')
    ...
    // SETに戻し、選択肢を選んでから2件目のルール(対象フィールド未選択のまま)を追加すると
    // 保存時バリデーションでエラーになることを確認する。
    ...
    await (await ruleRows[1].$('.js-rule-remove')).click();
    ...
    const reloadedFilterAction = await page.$eval(
      '.js-filter-action',
      (el) => el.value,
    );
    const reloadedTarget = await page.$eval('.js-target', (el) => el.value);
    expect(reloadedFilterAction).toBe(fixtures.PROCESS_ACTIONS.start.name);
    expect(reloadedTarget).toBe(fixtures.FIELD_CODES.radioTarget);
  });
});
